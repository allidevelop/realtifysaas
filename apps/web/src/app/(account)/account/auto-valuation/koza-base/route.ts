...
// Міст до autovalue koza-base: база шаблонів-козлів (готових звітів по домах).
// Гейт: вхід + доступ до auto-valuation. Завантаження квоту НЕ списує.
...
export async function GET() {
  ...
  const res = await autovalueFetch('/api/koza-base', cookie)
...
export async function POST(req: Request) {
  ...
  if (!(file instanceof File) || file.size === 0) {
    return Response.json({ error: 'Потрібен файл-звіт (.doc/.docx).' }, { status: 400 })
  }
  const out = new FormData()
  out.append('file', file, file.name || 'koza.doc')
  ...
  const res = await autovalueFetch('/api/koza-base', cookie, { method: 'POST', body: out })
  return new Response(await res.text(), { status: res.status, headers: { 'Content-Type': 'application/json' } })
}
